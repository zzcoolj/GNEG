...
grid_searcher = nsg.NegativeSamplesGenerator(ns_folder='output/intermediate data/negative_samples/',
                                             valid_vocabulary_path='output/intermediate data/dicts_and_encoded_texts/valid_vocabulary_min_count_5_vocab_size_10000.txt')
grid_searcher.many_to_many(encoded_edges_count_file_folder='output/intermediate data/graph/', directed=False, t_max=5, process_num=window_size-1, partial=False)
print('time in seconds:', common.count_time(start_time))
...
#                         workers=60, sg=sg, size=200, units=units)
...
#     'negative', 'NS file', 'Graph window size', 'Directed/Undirected', 't-random-walk', 'power',
#     # wordsim353
...
# negative = [5, 10, 15, 20]
# for i in range(4):
#     evaluation_result = gs.one_search(matrix_path=None, graph_index2wordId_path=None, power=None, ns_mode_pyx=0, negative=negative[i])
#     df.loc[i] = [str(negative[i])] + evaluation_result
# writer = pd.ExcelWriter('output/intermediate data/negative_samples_partial/' + 'negatives.xlsx')
